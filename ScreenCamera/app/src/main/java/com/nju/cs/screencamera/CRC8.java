...
public class CRC8 {
    ...
    /**
     * 定制的CRC8计算方法
     * 进行CRC8计算的为int型整数的低16位
     *
     * @param data 需要计算CRC8的数据
     * @return 返回CRC8校验值
     */
    public static int calcCrc8(int data) {
        byte[] b = new byte[]{
                (byte) (data >> 8),
                (byte) (data)
        };
        return calcCrc8(b, 0, 2, (byte) 0) & 0xff;
    }

    /**
     * 实际进行CRC8计算的方法
     *
     * @param data   需要计算CRC8数据的byte数组格式
     * @param offset 数组偏移值
     * @param len    计算的数组元素个数
     * @param preval 递归参数,第一次时一般为0
     * @return 返回CRC8校验值
     */
