 ...
 */
public class SingleImgToFileZoom extends MediaToFileZoom {
     ...
     * @param handler   实例
     */
    public SingleImgToFileZoom(TextView debugView, TextView infoView, Handler handler) {
    ...
    public void singleImg(String filePath) {
        ...
        bitmap.recycle();
        updateInfo("正在识别...");
        RGBMatrixZoom rgbMatrix=null;
        ArrayDataDecoder dataDecoder=null;
        int fileByteNum=-1;
        int[] border=null;
        try {
            rgbMatrix = new RGBMatrixZoom(byteBuffer.array(), bitmap.getWidth(), bitmap.getHeight(),border);
